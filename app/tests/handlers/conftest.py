...
from app.logic.feedback import FEEDBACK_STEPS_MSG
...
class FeedbackNameForm(TypedDict):
...
class FeedbackTextData(FeedbackNameForm):
...
def feedback_data(
    ...
    data: FeedbackData = {
        ...
        "expected_answer_msg": fixt_feedback_model.ANSWER_MSG.format(feedback_type_rus=feedback_type_rus),
...
def feedback_name_form_data(
    ...
    msg = fixt_feedback_model.NAME_FORM_MSG.format(name=name.capitalize(), feedback_type_rus="предложение")
    mock_state_clean.get_data.return_value = {
        "feedback_type_rus": "Предложение",
        "bot_message_id": mock_message.message_id,
    }

    return {"message": mock_message, "expected_text": msg, "mock_state": mock_state_clean}


@pytest.fixture(params=["Тестовое сообщение."])
def feedback_text_form_data(
                        request: pytest.FixtureRequest,
                        mock_state_clean: AsyncMock,
                        fixt_feedback_model: FeedbackModel,
                        mock_message: AsyncMock,
    ...
    Args:
        request: параметры для теста.
        mock_state_clean: Очищенный мок объекта состояния памяти FSMContext.
        fixt_feedback_model: Фикстура модели обратной связи.
        mock_message: мок Message.
    """
    text = request.param
    client_name = "alex"
    ...
    mock_message.answer.return_value = mock_message
    mock_state_clean.get_data.return_value = {"bot_message_id": mock_message.message_id, "name": client_name}
    answer_text_to_client = fixt_feedback_model.prepare_text(client_name, text)

    return {
        "message": mock_message,
        "expected_text": text,
        "mock_state": mock_state_clean,
        "expected_answer": answer_text_to_client,
    }
...
@pytest.fixture()
def mock_fms_context_with_get_data(mock_state_clean: AsyncMock) -> AsyncMock:
    ...
    """
    mock_state_clean.get_data.return_value = {}
    return mock_state_clean


@pytest.fixture()
def mock_message_user_configure_for_cmd_start(mock_message: AsyncMock, test_data: dict[str, bool | int]) -> AsyncMock:
...
@pytest.fixture()
def mock_wait_typing() -> AsyncMockGenerator:
    """Мокаем wait_typing."""
...
@pytest.fixture()
def mock_wait_typing_feedback() -> AsyncMockGenerator:
    """Мокаем wait_typing."""
    with patch("app.logic.feedback.wait_typing", new_callable=AsyncMock) as m:
        yield m
...
@pytest.fixture()
def mock_calback_with_params_drinks_point(mock_callback: AsyncMock) -> CallbackPointHint:
...
@pytest.fixture()
def mock_state_with_params(mock_state_clean: AsyncMock) -> AsyncMock:
...
@pytest.fixture()
def mock_state_with_params_coffee_item(mock_state_clean: AsyncMock) -> AsyncMock:
    """Конфигурируем параметрами мок FSMContext.

    Args:
        mock_state_clean: Очищенный мок объекта состояния памяти FSMContext.
    """
    mock_state_clean.get_data.return_value = {"point_id": 1}
    return mock_state_clean
...
@pytest.fixture()
def mock_state_with_params_final_feedback(mock_state_clean: AsyncMock) -> AsyncMockGenerator:
    """Фикстура подготавливает состояние памяти, эмитация завершения прохождения опроса.
