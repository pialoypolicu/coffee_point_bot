...
async def test_feedback_type_form(feedback_data: FeedbackData, mock_state: AsyncMock) -> None:
    ...
    await feedback_type_form(callback, mock_state)
    ...
    callback.message.reply.assert_awaited_once_with(  # callback.message.reply был вызван с правильными аргументами
        expected_msg,
...
async def test_feedback_name_form(feedback_name_form_data: FeedbackNameForm) -> None:
    ...
    message.reply.assert_awaited_once_with(expected_msg,
                                           reply_markup=back_to_start_keyboard,
                                           parse_mode=ParseMode.MARKDOWN_V2,)
...
async def test_feedback_text_form(feedback_text_form_data: FeedbackTextData, mock_state: AsyncMock) -> None:
    ...
    message.reply.assert_awaited_once_with("Загрузите фотографию\\. \\(*Не обязательно*\\.\\)",
